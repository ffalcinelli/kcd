...
pub fn run(input_dir: PathBuf) -> Result<()> {
        ...
        for entry in fs::read_dir(&roles_dir)? {
            ...
            if path.extension().is_some_and(|ext| ext == "yaml") {
                let content = fs::read_to_string(&path)
                    .context(format!("Failed to read role file {:?}", path))?;
                let role: RoleRepresentation = serde_yaml::from_str(&content)
                    .context(format!("Failed to parse role file {:?}", path))?;
    ...
    if clients_dir.exists() {
        for entry in fs::read_dir(&clients_dir)? {
            ...
            if path.extension().is_some_and(|ext| ext == "yaml") {
                let content = fs::read_to_string(&path)
                    .context(format!("Failed to read client file {:?}", path))?;
                let client: ClientRepresentation = serde_yaml::from_str(&content)
                    .context(format!("Failed to parse client file {:?}", path))?;

                if client.client_id.is_none()
                    || client.client_id.as_deref().unwrap_or("").is_empty()
                {
    ...
    if idps_dir.exists() {
        for entry in fs::read_dir(&idps_dir)? {
            ...
            if path.extension().is_some_and(|ext| ext == "yaml") {
                let content = fs::read_to_string(&path)
                    .context(format!("Failed to read idp file {:?}", path))?;
                let idp: IdentityProviderRepresentation = serde_yaml::from_str(&content)
                    .context(format!("Failed to parse idp file {:?}", path))?;

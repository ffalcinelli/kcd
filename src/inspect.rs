...
pub async fn run(client: &KeycloakClient, output_dir: PathBuf) -> Result<()> {
    if !fs::try_exists(&output_dir).await.context("Failed to check output directory")? {
    ...
    fs::write(output_dir.join("realm.yaml"), realm_yaml).await.context("Failed to write realm.yaml")?;
    ...
    if !fs::try_exists(&clients_dir).await.context("Failed to check clients directory")? {
        fs::create_dir_all(&clients_dir).await.context("Failed to create clients directory")?;
    }
    for client_rep in clients {
        ...
        fs::write(&path, yaml).await.context(format!("Failed to write client {}", name))?;
    }
    ...
    if !fs::try_exists(&roles_dir).await.context("Failed to check roles directory")? {
        fs::create_dir_all(&roles_dir).await.context("Failed to create roles directory")?;
    }
    for role in roles {
        let name = &role.name;
        let filename = format!("{}.yaml", sanitize(name));
        ...
        fs::write(&path, yaml).await.context(format!("Failed to write role {}", name))?;
    }
    ...
    if !fs::try_exists(&scopes_dir).await.context("Failed to check client-scopes directory")? {
        fs::create_dir_all(&scopes_dir).await.context("Failed to create client-scopes directory")?;
    }
    for scope in client_scopes {
        ...
        fs::write(&path, yaml).await.context(format!("Failed to write client scope {}", name))?;
    }
    ...
    if !fs::try_exists(&groups_dir).await.context("Failed to check groups directory")? {
        fs::create_dir_all(&groups_dir).await.context("Failed to create groups directory")?;
    }
    for group in groups {
        ...
        fs::write(&path, yaml).await.context(format!("Failed to write group {}", name))?;
    }
    ...
    if !fs::try_exists(&users_dir).await.context("Failed to check users directory")? {
        fs::create_dir_all(&users_dir).await.context("Failed to create users directory")?;
    }
    for user in users {
        ...
        fs::write(&path, yaml).await.context(format!("Failed to write user {}", username))?;
    }
    ...
    if !fs::try_exists(&flows_dir).await.context("Failed to check authentication-flows directory")? {
        fs::create_dir_all(&flows_dir).await.context("Failed to create authentication-flows directory")?;
    }
    for flow in flows {
        ...
        fs::write(&path, yaml).await.context(format!("Failed to write authentication flow {}", alias))?;
    }
    ...
    if !fs::try_exists(&actions_dir).await.context("Failed to check required-actions directory")? {
        fs::create_dir_all(&actions_dir).await.context("Failed to create required-actions directory")?;
    }
    for action in actions {
        ...
        fs::write(&path, yaml).await.context(format!("Failed to write required action {}", alias))?;
    }
    ...
    if !fs::try_exists(&components_dir).await.context("Failed to check components directory")? {
        fs::create_dir_all(&components_dir).await.context("Failed to create components directory")?;
    }
    for component in components {
        ...
        fs::write(&path, yaml).await.context(format!("Failed to write component {}", name))?;
    }

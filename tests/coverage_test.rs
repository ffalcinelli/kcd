...
use app::{plan, apply, inspect, clean};
...
async fn test_plan_edge_cases() {
    ...
    let res = plan::run(&client, workspace_dir.join("non-existent"), false, false, &[]).await;
    assert!(res.is_err());
    ...
    plan::run(&client, workspace_dir.clone(), false, false, &[]).await.unwrap();
    
    // 5. Test plan_realm with 404 (realm doesn't exist on server)
    // "new-realm" should return 404 in my mock server
    plan::run(&client, workspace_dir.clone(), false, false, &["new-realm".to_string()]).await.unwrap();

    // 6. Test with invalid YAML
    fs::write(realm_dir.join("invalid.yaml"), "invalid: [yaml").unwrap();
    let res = plan::run(&client, workspace_dir.clone(), false, false, &["new-realm".to_string()]).await;
...
async fn test_apply_edge_cases() {
    ...
    // 4. Test auto-discovery of realms
    apply::run(&client, workspace_dir.clone(), &[], true).await.unwrap();

    // 5. Test with empty .kcdplan
    fs::write(workspace_dir.join(".kcdplan"), "[]").unwrap();
    apply::run(&client, workspace_dir.clone(), &["test-realm".to_string()], true).await.unwrap();

    // 6. Test with invalid YAML in roles dir
    let roles_dir = realm_dir.join("roles");
    fs::create_dir_all(&roles_dir).unwrap();
    fs::write(roles_dir.join("invalid.yaml"), "invalid: [yaml").unwrap();
    let res = apply::run(&client, workspace_dir.clone(), &["test-realm".to_string()], true).await;
    assert!(res.is_err());
}
...
async fn test_check_keys_drift() {
    ...
    plan::run(&client, workspace_dir, true, false, &["test-realm".to_string()]).await.unwrap();
}
...
async fn test_inspect_edge_cases() {
    ...
    inspect::run(&client, workspace_dir.clone(), &[], true).await.unwrap();
    ...
    inspect::run(&client, workspace_dir.clone(), &["test-realm".to_string()], true).await.unwrap();
    ...
    let res = inspect::run(&bad_client, workspace_dir.clone(), &["test-realm".to_string()], true).await;
    assert!(res.is_err());
}
...
async fn test_clean_edge_cases() {
    ...
    clean::run(workspace_dir.clone(), true, &["some_file.txt".to_string()]).await.unwrap();

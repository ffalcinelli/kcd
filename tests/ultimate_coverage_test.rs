...
async fn test_ultimate_coverage() {
    ...
    // Run Plan
    plan::run(&client, workspace_dir.clone(), false, false, &["test-realm".to_string()]).await.unwrap();
    
    // Run Apply
    apply::run(&client, workspace_dir.clone(), &["test-realm".to_string()], true).await.unwrap();

    // Run Drift
    plan::run(&client, workspace_dir.clone(), true, false, &["test-realm".to_string()]).await.unwrap();
}
